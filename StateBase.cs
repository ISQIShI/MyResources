...
namespace StateMachineFrame
{
    ...
    public abstract class StateBase<TStateName>
    {
        ...
        public static readonly StateBase<TStateName> EmptyState = new EmptyState<TStateName>();
        ...
        public StateBase<TStateName> AddStateChangeInfo(TStateName targetName, Func<bool> changeCondition, bool autoAddLackingState, string conditionSign = null)
        {
            ...
            {
                stateChangeInfos = new List<StateChangeInfo<TStateName>>();
            }
            //如果已经存在相同的状态切换信息,则只添加条件
            var temp = stateChangeInfos.Find((info) => (info.targetStateName.Equals(targetName)) && (info.conditionSign == conditionSign));
            if (temp != null)
            {
            ...
            //否则添加新的状态切换信息
            stateChangeInfos.Add(new StateChangeInfo<TStateName>(stateName, targetName, changeCondition, conditionSign));
            return this;
        }
        ...
        public void RemoveStateChangeInfo(TStateName targetName, string conditionSign = null)
        {
            ...
            var temp = stateChangeInfos.RemoveAll((info) => (info.targetStateName.Equals(targetName)) && (info.conditionSign == conditionSign));
        ...
        public void RemoveWholeStateChangeInfo(TStateName targetName)
        {
            ...
            var temp = stateChangeInfos.RemoveAll((info) => info.targetStateName.Equals(targetName));
        ...
        protected void MergeStateChangeInfos(StateBase<TStateName> stateA, StateBase<TStateName> stateB, bool isAaddB)
        {
            if (stateA == null || stateB == null) return;
            if (isAaddB) stateA.stateChangeInfos.AddRange(stateB.stateChangeInfos);
            else stateA.stateChangeInfos.InsertRange(0, stateB.stateChangeInfos);
        }

        /// <summary>
        /// 检查是否可以切换状态,若可以则返回目标状态名,否则返回null
        /// </summary>
        /// <returns></returns>
        public object CheckStateChange()
        {
